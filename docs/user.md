...
## Running AnatQC
While you are welcome to install AnatQC and all of it's dependencies manually, 
using one of the prebuilt containers is the most reliable way to run AnatQC. The
remainder of this section will assume that you are running AnatQC using 
an [Apptainer/Singularity][Apptainer] container.
...
## XNAT Integration
### Tagging your scans
The [get][Get mode] mode of AnatQC is able to pull your data down from an [XNAT][] 
...
!!! note "Linking a `T1w` to its corresponding `vNav`"
    ...
    before its correspinding `T1w` scan, it feels wrong to hardcode that 
    ...
    numbers when you are entering the tags. For example, the vNav tag 
    `#T1w_MOVE_001` would correspond to the T1w scan `#T1w_001`.
